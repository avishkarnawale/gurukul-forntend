...
function Page() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <PageHeader
        title="Notes"
        subtitle="Study materials for students"
        action={<NewNote onDone={() => qc.invalidateQueries({ queryKey: ["admin-notes"] })} />}
      />
        ...
        {(data ?? []).map((n) => (
          <div key={n.id} className="card-elevated p-5">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              {n.classes?.name ?? "All classes"} · {n.subjects?.name}
            </p>
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await uploadNote({
        ...
        ...form,
        targetClass:
          form.targetClass && form.targetClass !== "__all__" ? form.targetClass : undefined,
      });
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div>
            ...
            <Select
              ...
              onValueChange={(v) => setForm({ ...form, targetClass: v })}
            >
              <SelectTrigger>
