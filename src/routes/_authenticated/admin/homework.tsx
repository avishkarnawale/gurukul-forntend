...
function Page() {
  ...
  const { data: hw } = usePortalQuery({ queryKey: ["admin-homework"], queryFn: fetchHomeworkAdmin });
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <PageHeader title="Homework" subtitle="Assignments across all classes" action={<NewHomework onDone={() => qc.invalidateQueries({ queryKey: ["admin-homework"] })} />} />
      <div className="grid gap-3 md:grid-cols-2">
        {(hw ?? []).map((h) => (
          <div key={h.id} className="card-elevated p-5">
            <div className="flex items-center justify-between gap-2">
              <span className="rounded-full bg-primary-soft px-2.5 py-1 text-[11px] font-semibold text-primary">{h.subjects?.name ?? "—"}</span>
...
function NewHomework({ onDone }: { onDone: () => void }) {
  ...
  const [form, setForm] = useState({ class: "", subject: "", title: "", description: "", due_date: "" });
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild><Button><Plus className="mr-2 h-4 w-4" /> Add Homework</Button></DialogTrigger>
      <DialogContent>
