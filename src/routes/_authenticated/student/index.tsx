    ...
    <QueryState loading={isLoading} error={isError ? (error as Error).message : null} onRetry={() => refetch()}>
      <div className="mx-auto max-w-7xl">
        <PageHeader
          ...
          subtitle={student ? `${student.classes?.name ?? ""} · Roll ${student.roll_number}` : "Loading…"}
        />

        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <StatCard label="Attendance" value={`${stats?.attendance.pct ?? 0}%`} hint={`${stats?.attendance.present ?? 0} / ${stats?.attendance.total ?? 0} days`} icon={Calendar} accent="success" />
          <StatCard label="Pending Homework" value={stats?.homework.length ?? 0} hint="Upcoming deadlines" icon={BookOpen} accent="primary" />
          <StatCard label="Fees Due" value={`₹${(stats?.fees.due ?? 0).toLocaleString()}`} hint="Across all terms" icon={Wallet} accent="warning" />
          <StatCard label="Notices" value={stats?.notices.length ?? 0} hint="New announcements" icon={Bell} accent="muted" />
        </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <div className="card-elevated p-6">
            ...
            <div className="mt-4 space-y-2">
              ...
              {!stats?.homework.length && <EmptyState title="No homework yet" hint="You're all caught up." />}
            </div>
          </div>
          <div className="card-elevated p-6">
              ...
              {(stats?.notices ?? []).map((n) => (
                <div key={n.id} className="rounded-xl border border-border bg-card p-3">
                  <div className="flex items-center justify-between">
                    ...
                    {n.pinned && <span className="rounded-full bg-primary-soft px-2 py-0.5 text-[10px] font-semibold text-primary">PINNED</span>}
                  </div>
